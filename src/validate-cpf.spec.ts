...
describe('testes do validador de CPF', () => {
  ...
  it.each(validCpfs)('deve retornar "true" para o CPF válido %s', (cpf: string) => {
    // When
  ...
  it.each(invalidCpfs)('deve retornar "false" para o CPF inválido %s', (cpf: string) => {
    const isValid = validateCpf(cpf)
  ...
  it('deve retornar uma exception para valores vazios ("null", "undefined")', () => {
    ...
    expect(() => validateCpf(cpf)).toThrow(new Error('o valor para CPF está vazio'))
  })

  it('deve retornar uma exception para valores que não sejam do tipo "string"', () => {
    const cpf = 98765432100 as unknown as string
    expect(() => validateCpf(cpf)).toThrow(new Error('o valor para CPF deve ser do tipo string'))
  })

  it('deve retornar uma exception para CPFs com padrão incorreto', () => {
    const cpf = '111;444.777-35'
    expect(() => validateCpf(cpf)).toThrow(new Error('o valor para CPF é diferente do padrão xxx.xxx.xxx-xx'))
  })
})
